...
def fetch_page(url: str, headers: dict) -> BeautifulSoup:
    ...
    if response.status != 200:
        raise ConnectionError("Unable to get the url response")
...
if __name__ == "__main__":
    stat_output_file = "pstats-cumulative.txt"
    sort_mode = "cumtime"
    top_number = 5
    ...
    try:
        with open(stat_output_file, "w", encoding="utf-8") as f:
            ...
            stats.strip_dirs().sort_stats(sort_mode)
            stats.print_stats(top_number)
    except Exception as e:
